...
public class BossAI2 : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        Manager.SoundManager.PlayBackGroundSound("Boss_Battle");
    }
    ...
    void Die()
    {
        ...
        onDeath?.Invoke();
        if (playAfterBossBgmOnDeath)
        {
            Manager.SoundManager.PlayBackGroundSound("Boss_after");
        }
        TriggerDeathDialogue();
        DialogueUI.Instance.StartTalk(talkOnDeath_Main, talkOnDeath_other);
        DialogueUI.Instance.End += (() => Manager.SceneTransitionManager.Instance.LoadScene("End"));
